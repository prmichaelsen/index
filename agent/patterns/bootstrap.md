...
- **`prepublishOnly`**: Automatically runs before `npm publish` to ensure fresh build
  - Cleans old build artifacts
  - Rebuilds everything from scratch
  - Prevents publishing stale or incomplete builds
- **`publishConfig`**: Set `"access": "public"` for scoped packages (@scope/name)
